...
import { Code, KeyRound, Mail, Loader2, ArrowRight } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Ensure app user record exists for this Better Auth user
      if (data?.user) {
  ...
  return (
    <div className="min-h-screen text-white font-sans selection:bg-white/20 overflow-x-hidden relative flex flex-col justify-center items-center px-4">
      <AnimatedBackground />
      ...
      <div className="w-full max-w-md flex flex-col gap-4 relative z-10 animate-in zoom-in-95 duration-500">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-neutral-400 mt-2 text-base">Enter your details to resume your journey.</p>
        </div>
        ...
        <p className="mt-8 text-center text-neutral-400 text-sm">
          ...
          <Link href="/signup" className="text-blue-400 hover:text-blue-300 transition-colors font-medium hover:underline underline-offset-4">
            Create an account
          </Link>
        </p>
      </div>
