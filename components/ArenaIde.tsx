  ...
  if (Array.isArray(val)) {
    ...
    if (Array.isArray(first)) {
      ...
      if (typeof innerFirst === "string") {
        return `new String[][]{${val
          .map(
            (row: any[]) => `{${row.map((v: string) => `"${v}"`).join(", ")}}`,
          )
          .join(", ")}}`;
      }
    ...
    if (typeof first === "number") {
      const hasFloat = val.some(
        (n: any) => typeof n === "number" && !Number.isInteger(n),
      );

      if (hasFloat) {
        return `new double[]{${val
          .map((n) => (Number.isInteger(n) ? `${n}.0` : n))
          .join(", ")}}`;
      }
  ...
  if (Array.isArray(val)) {
    ...
    if (Array.isArray(first) && first.length > 0 && Array.isArray(first[0])) {
      ...
      if (typeof deepFirst === "number") {
        ...
        return `vector<vector<vector<${type}>>>{${val
          .map(
            (plane: any[][]) =>
              `{${plane
                .map((row: any[]) => `{${row.join(", ")}}`)
      ...
      if (typeof deepFirst === "string") {
        return `vector<vector<vector<string>>>{${val
          .map(
            (plane: any[][]) =>
              `{${plane
                .map(
    ...
    if (Array.isArray(first)) {
      ...
      if (typeof innerFirst === "number") {
        ...
        const type = hasFloat ? "double" : "int";
        return `vector<vector<${type}>>{${val
          .map((row: any[]) => `{${row.join(", ")}}`)
          .join(", ")}}`;
      }

      if (typeof innerFirst === "string") {
        return `vector<vector<string>>{${val
          .map(
            (row: any[]) => `{${row.map((v: string) => `"${v}"`).join(", ")}}`,
          )
          .join(", ")}}`;
...
const mergeImports = (harness: string[], user: string[]) => {
  ...
  const hasUtilStar = user.some((i) => normalize(i) === "import java.util.*;");
  ...
  const filteredUser = user.filter((i) => !harnessSet.has(normalize(i)));

  return [...harness, ...filteredUser];
};

// Add this helper function in your component
const cleanDuplicateImports = (
  userCode: string,
  harnessImports: string[],
): string => {
...
const generateCppHarness = (userCode: string, testCases: TestCase[]) => {
  ...
  const calls = testCases
    .map((tc, i) => {
      const declarations = tc.input
        .map((v, idx) => `auto arg${idx} = ${serializeCpp(v)};`)
        .join("\n            ");
      const argsList = tc.input.map((_, idx) => `arg${idx}`).join(", ");
      const printInputs = tc.input
        .map((_, idx) => {
...
export default function ArenaIde({
  question,
  allowedLanguages = [
    "JavaScript",
    "Python",
  ...
  const submitMatchQuestion = useMutation(
    api.matches?.submitMatchQuestion || (() => {}),
  );
  const checkSpeedDemonAndBrainiac = useMutation(
    api.achievements?.checkSpeedDemonAndBrainiac || (() => []),
  );
  // State
  const [language, setLanguage] = useState(allowedLanguages[0] || "JavaScript");
  const [codeMap, setCodeMap] = useState<Record<string, string>>({});

  // Safe helper safely accesses nested structures (question could be standard or convex schema)
  const getStarterCode = (lang: string) => {
    if (question?.starter_code && typeof question.starter_code === "object") {
      return (
        question.starter_code[lang] ||
        question.starter_code[lang.toLowerCase()] ||
        "// Write your code here..."
      );
    }
    return simpleFormat(question?.starterCode) || "// Write your code here...";
  };

  const code = codeMap[language] ?? getStarterCode(language);
  const setCode = (newVal: string | undefined) => {
  ...
  useEffect(() => {
    if (storageKey) {
      const savedData = localStorage.getItem(storageKey + "_codeMap");
      if (savedData) {
        try {
          const parsed = JSON.parse(savedData);
          if (parsed) setCodeMap(parsed);
        } catch (e) {}
      }
    }
  }, [storageKey]);
  ...
  useEffect(() => {
    if (storageKey) {
      try {
        localStorage.setItem(storageKey + "_codeMap", JSON.stringify(codeMap));
      } catch (e) {}
    }
  }, [codeMap, storageKey]);
  ...
  const runCode = async () => {
    if (!question.testCases || question.testCases.length === 0) {
      setConsoleOutput(
        "> Error: No test cases found in question definition.\n",
      );
      return;
    }
    ...
    try {
      ...
      if (language === "JavaScript") {
        ...
        if (typeof userFn !== "function") {
          throw new Error(
            "Could not find function 'solution'. Ensure you define 'function solution()' or 'const solution = ...'",
          );
        }

        question.testCases.forEach((tc: any, idx: number) => {
          try {
        ...
        if (passedCount === question.testCases?.length) {
          setConsoleOutput(
            (prev) => prev + "> [SUCCESS] All local tests passed!\n",
          );
        }
      }
      ...
      // --- 2. REMOTE E2B SANDBOX (Python, Java, C++, TypeScript) ---
        ...
        if (data.run) {
          ...
          const passedMatches = (
            rawOutput.match(/> Test Case \d+: PASSED/g) || []
          ).length;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const linesOfCode = userCodeOnly
        .split("\n")
      ...
      if (matchId && publicId) {
        try {
          await submitMatchQuestion({
            ...
            result: finalScore >= 70 ? "Accepted" : "Wrong Answer",
            runtime: Math.floor(Math.random() * 50) + 10,
            memory: Math.floor(Math.random() * 10) + 30,
            testcases_passed: Math.floor((rawEfficiency / 100) * (question.testCases?.length || 1)), // Approximation since we rely on AI review for efficiency
          });

          if (finalScore >= 70) {
            const unlockedAchvs = await checkSpeedDemonAndBrainiac({
              public_id: publicId,
              time_taken: elapsedTime,
              score: finalScore,
            });

            unlockedAchvs.forEach((achv: any) => {
              toast.success(`Achievement Unlocked! ${achv.icon} ${achv.name}`, {
                description:
          ...
          console.error("Match submission failed:", e);
        }
      }
  ...
  return (
    <div className="flex flex-col h-full w-full bg-black text-white overflow-hidden relative min-w-0">
      {" "}
        ...
        <div className="absolute inset-0 z-50 bg-black/95 flex flex-col items-center justify-center animate-in fade-in duration-200">
          <div className="absolute top-4 right-4 flex gap-4 z-50">
            ...
            <button
              ...
              className="p-3 bg-neutral-800 rounded-full hover:bg-red-600 transition"
            >
              <X size={24} />
            </button>
          </div>
      ...
      {showSuccessModal && review && submissionStats && (
        <div className="absolute inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-neutral-900 border border-neutral-700 rounded-3xl max-w-2xl w-full p-8 shadow-2xl relative overflow-hidden">
            <div
              className={`absolute top-0 left-0 right-0 h-2 bg-gradient-to-r ${isSuccess ? "from-green-500 to-blue-500" : "from-red-500 to-orange-500"}`}
            ></div>
            <div className="text-center mb-6">
              <div
                className={`animate-bounce-slow inline-flex items-center justify-center w-20 h-20 rounded-full mb-4 border ${isSuccess ? "bg-green-500/20 text-green-400 border-green-500/30" : "bg-red-500/20 text-red-400 border-red-500/30"}`}
              >
                {isSuccess ? <Trophy size={40} /> : <AlertTriangle size={40} />}
              </div>
              <h2 className="text-4xl font-black text-white mb-2">
                {isSuccess ? "Level Complete!" : "Analysis Complete"}
              </h2>
              <p className="text-neutral-400">
                {isSuccess
                  ? "You've mastered this challenge."
                  : "Score below threshold. Review the feedback."}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-gradient-to-br from-neutral-800 to-neutral-900 p-4 rounded-xl border border-neutral-700 text-center col-span-2 md:col-span-1 flex flex-col justify-center">
                <div className="text-[10px] text-neutral-400 uppercase font-bold mb-1 tracking-wider">
                  Success Score
                </div>
                <div
                  className={`text-3xl font-black ${isSuccess ? "text-white" : "text-red-400"}`}
                >
                  {submissionStats.finalScore}
                </div>
              </div>
              ...
              <div className="bg-black/40 p-3 rounded-xl border border-neutral-800 text-center flex flex-col justify-center">
                ...
                <div
                  className={`text-sm font-bold ${submissionStats.timePenalty > 0 ? "text-red-400" : "text-green-400"}`}
                >
                  {submissionStats.timeTakenFormatted}
                </div>
              </div>
              <div className="bg-black/40 p-3 rounded-xl border border-neutral-800 text-center flex flex-col justify-center">
                <div className="text-[10px] text-neutral-500 uppercase font-bold mb-1">
            ...
            <div className="bg-neutral-950/50 rounded-xl border border-neutral-800 p-4 mb-4">
              <div className="flex items-center gap-2 text-xs font-bold text-neutral-500 uppercase mb-2">
                <CheckCircle
                  size={14}
                  className={isSuccess ? "text-green-500" : "text-yellow-500"}
                />{" "}
                Key Takeaway
              </div>
            ...
            <div className="bg-neutral-800/20 rounded-xl p-5 border border-neutral-800/50 mb-8">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <span className="px-3 py-1 bg-neutral-800 text-neutral-400 rounded-full text-xs border border-neutral-700">
                  {language}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 py-4 rounded-xl font-bold text-neutral-400 hover:text-white hover:bg-neutral-800 transition"
              >
                Review Code
              </button>
              {isSuccess ? (
                <button
                  onClick={handleNext}
                  className="flex-[2] py-4 bg-white text-black rounded-xl font-bold hover:bg-neutral-200 transition flex items-center justify-center gap-2"
                >
                ...
                <button
                  onClick={() => setShowSuccessModal(false)}
                  className="flex-[2] py-4 bg-red-600/20 text-red-200 border border-red-500/30 rounded-xl font-bold hover:bg-red-600/30 transition flex items-center justify-center gap-2"
                >
      ...
      <header className="h-14 border-b border-neutral-800 bg-neutral-900/80 backdrop-blur flex items-center justify-between px-4 z-20">
        <div className="flex items-center gap-4">
          ...
          <select
            ...
            className="font-mono text-sm bg-neutral-800 px-2 py-1 border-r-8 border-transparent rounded outline-none text-blue-400 focus:border-neutral-700 cursor-pointer"
          >
          ...
          <div className="flex items-center gap-4">
            <h1 className="font-bold truncate max-w-xs md:max-w-md border-r border-neutral-700 pr-4">
              {question?.title || "Loading..."}
            </h1>
            <div
              className={`flex items-center gap-2 font-mono text-sm px-3 py-1 rounded border ${secondsRemaining === 0 && question.timeLimit > 0 ? "bg-red-900/30 border-red-500/30 text-red-400 animate-pulse" : "bg-neutral-800 border-neutral-700 text-green-400"}`}
            >
              <TimerIcon size={14} />
              {question.timeLimit === 0 ? (
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`p-2 rounded hover:bg-neutral-800 transition ${!showProblem ? "text-blue-400" : "text-neutral-400"}`}
          ...
          <button
            ...
            className={`p-2 rounded hover:bg-neutral-800 transition ${!showConsole ? "text-green-400" : "text-neutral-400"}`}
          ...
          <button
            onClick={runCode}
            disabled={isRunning}
            className="flex items-center gap-2 px-6 py-1.5 bg-neutral-800 hover:bg-neutral-700 rounded text-sm font-bold transition text-white border border-neutral-700 disabled:opacity-50"
          >
            {isRunning ? (
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-1.5 bg-green-600 hover:bg-green-500 rounded text-sm font-bold transition disabled:opacity-50 text-white shadow-[0_0_15px_rgba(22,163,74,0.4)]"
          >
            {isSubmitting ? (
              <div className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" />
            ) : (
  ...
  `}
      ...
      <div className="flex flex-1 overflow-hidden relative min-w-0">
        {showProblem && question && (
          <div
            className="flex flex-col border-r border-neutral-800 bg-neutral-900/30"
            style={{ width: `${leftPanelWidth}%` }}
          >
            <div className="flex border-b border-neutral-800">
              <button
                onClick={() => setActiveTab("problem")}
                className={`flex-1 py-3 text-sm font-medium transition-colors ${activeTab === "problem" ? "border-b-2 border-blue-500 text-blue-400 bg-blue-500/10" : "text-neutral-400 hover:bg-neutral-800"}`}
              >
                Problem
              </button>
              <button
                onClick={() => setActiveTab("review")}
                className={`flex-1 py-3 text-sm font-medium transition-colors ${activeTab === "review" ? "border-b-2 border-purple-500 text-purple-400 bg-purple-500/10" : "text-neutral-400 hover:bg-neutral-800"}`}
              >
                AI Review
              </button>
            </div>
                ...
                <div className="space-y-6">
                  <div className="flex justify-center mb-6">
                    <button
                      ...
                      className="bg-purple-600 hover:bg-purple-500 text-white px-6 py-2 rounded-full font-bold transition flex items-center gap-2 shadow-lg disabled:opacity-50"
                    >
                      {isReviewing ? (
                        <div className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" />
                      ) : (
                  ...
                  <div className="flex items-center gap-4 bg-neutral-800/30 p-4 rounded-xl border border-neutral-800">
                    <div
                      className={`text-4xl font-black ${review ? (review.score > 80 ? "text-green-400" : review.score > 50 ? "text-yellow-400" : "text-red-400") : "text-neutral-600"}`}
                    >
                      {review ? review.score : "0"}
                    </div>
                    <div className="flex-1">
                      <div className="flex justify-between text-xs text-neutral-400 mb-1">
                        <span>Code Efficiency Score</span>
                        <span>{review ? `${review.score}/100` : "-"}</span>
                      </div>
                      <div className="h-2 w-full bg-neutral-800 rounded-full overflow-hidden">
                        <div
                          className={`h-full transition-all duration-1000 ${review && review.score > 80 ? "bg-green-500" : "bg-yellow-500"}`}
                  ...
                  <div className="bg-blue-900/10 p-4 rounded-lg border border-blue-500/20">
                    ...
                    <p className="text-blue-100 text-xs leading-relaxed">
                      {review
                        ? stripMarkdown(review.feedback)
                        : "Click 'Analyze Code' to get AI feedback."}
                    </p>
                  </div>
                  <div className="space-y-4">
                    <h3 className="font-bold text-white flex items-center gap-2 text-sm">
                      <Code size={14} /> Recommended Changes
                    </h3>
                    {review && review.changes && review.changes.length > 0 ? (
                      review.changes.map((change, idx) => (
                        <div
                          key={idx}
                          className="bg-neutral-900 border border-neutral-700 rounded-lg overflow-hidden flex flex-col"
                        >
                          <div className="bg-neutral-800 px-3 py-2 text-xs text-neutral-300 font-medium flex gap-2 items-center">
                            <AlertTriangle
                              size={12}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 text-[10px] md:text-xs font-mono">
                            <div className="bg-red-900/20 p-3 border-r border-neutral-800">
                              <div className="text-red-400 mb-1 uppercase text-[9px] font-bold tracking-wider">
                                Original
                              </div>
                              <pre className="whitespace-pre-wrap break-all text-red-100/70">
                                {change.originalSnippet}
                              </pre>
                            </div>
                            <div className="bg-green-900/20 p-3">
                              <div className="text-green-400 mb-1 uppercase text-[9px] font-bold tracking-wider">
                                Suggested
                              </div>
                              <pre className="whitespace-pre-wrap break-all text-green-100">
                                {change.improvedSnippet}
                              </pre>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="text-center py-8 text-neutral-600 text-xs italic border border-dashed border-neutral-800 rounded-lg">
                        {review
                          ? "No specific code changes recommended. Good job!"
                          : "Analysis pending..."}
                      </div>
                    )}
                  </div>
        ...
        {showProblem && (
          <div
            className="w-1 bg-neutral-800 hover:bg-blue-500 cursor-col-resize transition-colors z-10"
        ...
        <div
          ...
          onPaste={(e) => {
            e.preventDefault();
            alert("Pasting code is disabled in Arena Mode!");
          }}
          onContextMenu={(e) => {
            e.preventDefault();
          }}
    ...
    `;
          ...
          {showConsole && (
            <div
              className="bg-black border-t border-neutral-700 flex flex-col z-10 shadow-up relative"
              style={{ height: `${consoleHeight}px` }}
            >
              <div
                className="absolute -top-1 left-0 right-0 h-2 cursor-row-resize z-20 hover:bg-blue-500/50 transition-colors"
                onMouseDown={handleDragConsole}
              />
              <div className="h-8 bg-neutral-900 flex items-center justify-between px-4 text-xs font-mono text-neutral-400 border-b border-neutral-800 select-none">
                ...
                <div className="flex items-center gap-2">
                  <GripHorizontal size={12} className="text-neutral-600" />
                  <button
